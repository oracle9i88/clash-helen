...
const Mihomo: React.FC = () => {
  ...
  const [hysteria2PortsInput, setHysteria2PortsInput] = useState(hysteria2Ports)
  const [hysteria2HopIntervalInput, setHysteria2HopIntervalInput] = useState(hysteria2HopInterval)
  ...
  return (
    <>
      ...
      <BasePage title={t('mihomo.title')}>
        ...
        <SettingCard>
          <SettingItem title={t('mihomo.hysteria2PortHopping')} divider>
            <Switch
              size="sm"
              isSelected={hysteria2PortHopping}
              onValueChange={async (v) => {
                await handleConfigChangeWithRestart('hysteria2PortHopping', v)
              }}
            />
          </SettingItem>
          {hysteria2PortHopping && (
            <>
              <SettingItem title={t('mihomo.hysteria2Ports')} divider>
                <Input
                  size="sm"
                  className="w-[200px]"
                  value={hysteria2PortsInput}
                  placeholder="20000-40000"
                  onValueChange={(v) => setHysteria2PortsInput(v)}
                  onBlur={async () => {
                    if (hysteria2PortsInput !== hysteria2Ports) {
                      await handleConfigChangeWithRestart('hysteria2Ports', hysteria2PortsInput)
                    }
                  }}
                />
              </SettingItem>
              <SettingItem title={t('mihomo.hysteria2HopInterval')}>
                <Input
                  size="sm"
                  className="w-[200px]"
                  type="number"
                  value={String(hysteria2HopIntervalInput)}
                  placeholder="30"
                  onValueChange={(v) => setHysteria2HopIntervalInput(parseInt(v) || 30)}
                  onBlur={async () => {
                    const val = hysteria2HopIntervalInput || 30
                    if (val !== hysteria2HopInterval) {
                      await handleConfigChangeWithRestart('hysteria2HopInterval', val)
                    }
                  }}
                />
              </SettingItem>
